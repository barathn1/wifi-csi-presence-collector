...
def compute_day_baseline_from_sessions(session_dirs: list[str], label: str, mode: str = "resampled",
                                        ...
                                        board_mac: str | None = None) -> DayBaseline:
    ...
    rather than mixing a raw-rate baseline with time-normalized windows. `board_mac` (default None)
    selects one receiver in a multi-receiver session -- see decode_csi.load_session."""
    amp_sum = amp_sumsq = phase_sum = phase_sumsq = None
    n = 0
    for session_dir in session_dirs:
        cache_path = cache_session(session_dir, mode, target_rate_hz=target_rate_hz, board_mac=board_mac)
        with np.load(cache_path) as d:
            amp, phase = d["amplitude"], d["phase"]
        if amp_sum is None:
            amp_sum, amp_sumsq = amp.sum(0), (amp.astype(np.float64) ** 2).sum(0)
