...
def _session_cache_path(session_dir_rel: str, mode: str, target_rate_hz: float | None = None,
                         board_mac: str | None = None) -> Path:
    ...
    # board_mac folded in too (2026-09-21 onward: 3 receivers per session_dir, see decode_csi.load_session)
    # so caches for different receivers of the same session never collide.
    if board_mac is not None:
        suffix += f"_{board_mac.replace(':', '').lower()}"
...
def cache_session(session_dir_rel: str, mode: str = "resampled", force: bool = False,
                   target_rate_hz: float | None = None, board_mac: str | None = None) -> Path | None:
    ...
    change to "resampled"'s existing behavior. `board_mac` selects one receiver in a multi-receiver
    session (see decode_csi.load_session); None (default) keeps every existing caller's behavior
    unchanged (single-receiver sessions, or "whichever receiver wrote the unprefixed file"). Returns
    None if the session has too few packets to have decoded at all."""
    ...
    out_path = _session_cache_path(session_dir_rel, mode, target_rate_hz, board_mac)
    if out_path.exists() and not force:
        return out_path

    session = load_session(REPO_ROOT / "data" / session_dir_rel, board_mac=board_mac)
    ...
    if mode == "resampled_timenorm":
        from ml.data_pipeline.time_resample import fix_clock_reset, resample_time_axis
        keep = fix_clock_reset(device_time_us)
        if not keep.all():
            amplitude, phase, rssi, device_time_us = (
                amplitude[keep], phase[keep], rssi[keep], device_time_us[keep])
...
def build_window_index(
    ...
    time_resample.py::compute_target_rate_hz. `board_mac` (default None) selects one receiver for every
    row -- correct as long as the manifest passed in only contains sessions where that receiver actually
    recorded (single-receiver sessions, or a multi-receiver manifest already filtered to one board_mac
    upstream, see train_day3_ch6_model.py::SINGLE_RECEIVER_MAC)."""
    manifest = manifest if manifest is not None else load_manifest()
    stride_packets = stride_packets or window_packets // 2

    rows = []
    for _, row in manifest.iterrows():
        cache_path = cache_session(row["session_dir"], mode, target_rate_hz=target_rate_hz, board_mac=board_mac)
