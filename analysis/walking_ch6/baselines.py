...
walking-only scope). kNN / linear-kernel SVM / RandomForest on the standardized handcrafted feature
...
MODELS = {
    ...
    "svm_rbf": lambda: SVC(kernel="rbf", C=1.0, gamma="scale", max_iter=3000),  # capped: walking data's
    # noisier/less-separable feature space made the uncapped solver take 15+ minutes on a single fold
    "random_forest": lambda: RandomForestClassifier(n_estimators=200, max_depth=8, random_state=0,
                                                      n_jobs=-1),
}


MAX_WINDOWS_PER_SESSION = 80  # cap so SVM fit time stays reasonable; windows overlap 50% anyway so this
...
def main():
    ...
    n_loso_perms = 3  # reduced further -- now 44 sessions (6 days) x SVM, LOSO cost scales with session count
