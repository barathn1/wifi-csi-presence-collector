...
# Ordinal ramp, blue steps light->dark = earlier->later day. Sized for up to 8 dates (this scope has 6);
# built as a function rather than a fixed dict since the channel-6 scope now spans 6 dates, not 3.
_DAY_RAMP_STEPS = ["#cde2fb", "#9ec5f4", "#6da7ec", "#3987e5", "#2a78d6", "#1c5cab", "#104281", "#0d366b"]


def day_color_map(dates) -> dict:
    dates = sorted(dates)
    n = len(dates)
    if n == 1:
        idx = [len(_DAY_RAMP_STEPS) // 2]
    else:
        idx = [round(i * (len(_DAY_RAMP_STEPS) - 1) / (n - 1)) for i in range(n)]
    return {d: _DAY_RAMP_STEPS[i] for d, i in zip(dates, idx)}
...
FIGURES_DIR = None  # set by caller scripts via analysis.walking_ch6 paths
