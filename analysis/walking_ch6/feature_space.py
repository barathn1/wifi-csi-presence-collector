...
from analysis.walking_ch6.load_data import WINDOW_INDEX_PATH, FEATURES_PATH, build
from analysis.walking_ch6.viz_common import IDENTITY_COLOR, PRIMARY_TEXT, day_color_map, identity_color, style_axes
...
def plot_by_day(window_index, pca, um) -> None:
    day_color = day_color_map(window_index["date"].unique())
    fig, axes = plt.subplots(1, 2, figsize=(12, 5.5))
    for i, (emb, ax, title) in enumerate([(pca, axes[0], "PCA"), (um, axes[1], "UMAP")]):
