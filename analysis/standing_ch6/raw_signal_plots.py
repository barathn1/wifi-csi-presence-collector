...
from analysis.standing_ch6.load_data import FIXED_BOARD, restricted_manifest
from analysis.standing_ch6.viz_common import (
    DAY_COLOR, IDENTITY_COLOR, MUTED_TEXT, PRIMARY_TEXT, SECONDARY_TEXT, identity_color, style_axes,
)
...
def plot_person_across_days(manifest, person_id: str) -> None:
    """One subplot per day this person has standing sessions, amplitude of EXAMPLE_SUBCARRIER over the
    ...
    fig.suptitle(f"{person_id}: raw CSI amplitude over time, every standing session by day", color=PRIMARY_TEXT)
    fig.tight_layout()
...
def plot_subcarrier_across_recordings(manifest, person_id: str, subcarriers=(20, 64, 100)) -> None:
    ...
    fig.suptitle(f"{person_id}: fixed subcarriers across every standing recording", color=PRIMARY_TEXT)
    fig.tight_layout()
...
def plot_mean_spectrum_profile(manifest) -> None:
    ...
    ax.set_title("Mean amplitude-vs-subcarrier profile, every standing session", color=PRIMARY_TEXT)
